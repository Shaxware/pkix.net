...
namespace System.Security.Cryptography.Pkcs {
	...
	public class PKCS7SignedMessage {
		List<X509Attribute> attributes;
		List<Oid> digestAlgs = new List<Oid>();
		List<X509CRL2> crls;
		X509Certificate2Collection certificates;
		List<X509CertificateRequest> requests;
		List<SignerInfo2> signerInfos;
		...
		public OidCollection DigestAlgorithms {
			get {
				if (digestAlgs == null) { return null; }
				OidCollection oids = new OidCollection();
				foreach (Oid oid in digestAlgs) {
					oids.Add(oid);
				}
				return oids;
			}
		...
		void decodeAttributes(IntPtr rgTaggedAttribute, UInt32 cTaggedAttribute) {
			...
			attributes = new List<X509Attribute>();
			...
			for (Int32 index = 0; index < cTaggedAttribute; index++) {
				...
				Attributes.Add(new X509Attribute(new Oid(attr.pszObjId), (Int32)attr.dwBodyPartID, bytes));
				rgValue += size;
			}
		}
		void decodeRequest(IntPtr rgTaggedRequest, UInt32 cTaggedRequest) {
			if (cTaggedRequest == 0) { return; }
			requests = new List<X509CertificateRequest>();
			...
			for (Int32 index = 0; index < cTaggedRequest; index++) {
				...
				requests.Add(new X509CertificateRequest(reqBytes));
				rgValue += size;
			}
		}
		// TODO
		...
		void decodeCrls(Wincrypt.CRYPTOAPI_BLOB blob) {
			...
			asn.MoveNext();
			do {
				crls.Add(new X509CRL2(asn.GetTagRawData()));
			} while (asn.MoveNextCurrentLevel());
		}
		void decodeSignerInfos(Wincrypt.CRYPTOAPI_BLOB blob) {
			if (blob.cbData == 0) { return; }
			signerInfos = new List<SignerInfo2>();
			...
			asn.MoveNext();
			do {
				signerInfos.Add(new SignerInfo2(asn.GetTagRawData(), certificates));
			} while (asn.MoveNextCurrentLevel());
		}
